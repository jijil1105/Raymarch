...
// 初期化処理
void DXApplication::OnInit(HWND hwnd, float elapsedTime, int width, int height)
{
	LoadPipeline(hwnd);
	LoadAssets(elapsedTime, width, height);
}
...
void DXApplication::LoadPipeline(HWND hwnd)
{
	...
	for (UINT index = 0; 1; ++index)
	{
		...
		HRESULT hr = dxgiFactory->EnumAdapters(index, &pAdapters[index]);
	...
	{
		// 生成可能なバージョンでループを抜ける
		if (D3D12CreateDevice(pSelectAdapter.Get(), lv, IID_PPV_ARGS(device_.ReleaseAndGetAddressOf()))==S_OK)
		{
			break;
		}
	}
...
void DXApplication::LoadAssets(float elapsedTime, int width, int height)
{
	// ルートシグネチャの生成
	{
		// ルートパラメータの生成
		// ディスクリプタテーブルの実体
		const UINT rangeNum = 2;
		CD3DX12_DESCRIPTOR_RANGE1 discriptorRanges[rangeNum] = {};
		...
		rootParameters[0].InitAsDescriptorTable(rangeNum, discriptorRanges, D3D12_SHADER_VISIBILITY_ALL); // 同一パラメータで2つ指定
		// サンプラーの生成
	...
	// パイプラインステートの生成
	{
		...
		D3DCompileFromFile(L"Assets/Shader/BasicVertexShader.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "vs_5_0", compileFlags, 0, &vsBlob, nullptr);
		D3DCompileFromFile(L"Assets/Shader/RaymarchPixel.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "ps_5_0", compileFlags, 0, &psBlob, nullptr);
	...
	// インデックスバッファビューの生成
	{
		const UINT indexBufferSize = sizeof(m_indices);
	...
	{
		ThrowIfFailed(LoadFromWICFile(L"Assets/Texture/test_image.png", DirectX::WIC_FLAGS_NONE, &metadata, scratchImg));
	...
	// 定数バッファービューの生成
	{
		...
		auto constDesc = CD3DX12_RESOURCE_DESC::Buffer((sizeof(cBuffer0) + 0xff) & ~0xff);
		ThrowIfFailed(device_->CreateCommittedResource(
			&constHeapProp,
			D3D12_HEAP_FLAG_NONE,
			&constDesc,
		...
		// 定数情報のコピー
		constBuffer_->Map(0, nullptr, (void**)&mapCBuffer_);
		mapCBuffer_->mat = worldMatrix_ * viewMatrix_ * projMatrix_;
		mapCBuffer_->elapsedTime = elapsedTime;
		mapCBuffer_->resolution = DirectX::XMFLOAT2(width, height);
	...
	{
		// テクスチャバッファの転送
		UpdateSubresources(commandList_.Get(), textureBuffer_.Get(), textureUploadBuffer.Get(), 0, 0, (UINT)textureSubresources.size(), textureSubresources.data());
...
// 更新処理
void DXApplication::OnUpdate(float elapseTime)
{
	// 回転させてみる
	//angle_ += 1.0f;
	...
	//mapCBuffer_->mat = worldMatrix_ * viewMatrix_ * projMatrix_;
	mapCBuffer_->elapsedTime = elapseTime;
}
...
void DXApplication::OnRender()
{
	...
	// コマンドリストの生成
	{
		...
		commandList_->DrawIndexedInstanced((UINT)sizeof(m_indices)/sizeof(m_indices[0]), 1, 0, 0, 0);                        // 描画
